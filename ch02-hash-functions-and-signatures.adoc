...
called _Schnorr_, featuring some new, useful properties. The intuition
we use here—signing is “encrypt the hash of the message with the
private key”—still works for Schnorr, even though the math underneath
is different. We cover Schnorr and the Taproot upgrade that introduced
it in <<ch12>>.
****
