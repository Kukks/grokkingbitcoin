...
[.inbitcoin]
.A second signature scheme: Schnorr
****
Since November 2021, Bitcoin has supported a second signature scheme
called _Schnorr_ alongside the one described in this chapter. Schnorr
signatures use the same private and public keys, but they have a
different internal structure that lets several signers combine their
work into a single signature. The intuition we use here—signing is
“encrypt the hash of the message with the private key”—still works
for Schnorr, even though the math underneath is different. We cover
Schnorr and the Taproot upgrade that introduced it in <<ch12>>.
****
