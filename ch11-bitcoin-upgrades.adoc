...
.Taproot
****
Taproot, deployed in 2021, is the only soft fork since segwit.
<<ch12>> covers what it does.
****

The mechanism used to deploy Taproot is called _Speedy Trial_. It’s
a stripped-down BIP9:

* _Short signaling window_—three months instead of a year. If miners
don’t lock it in within the window, the deployment fails and can be
re-attempted with different parameters.

* _90% lock-in threshold_ per retarget period.

* _Mandatory delayed activation_ if locked in. The rules take effect
at a fixed, pre-announced block height several months later, giving
wallets and exchanges a predictable runway.

[[fig1131]]
.Speedy Trial state machine.
image::{imagedir}/11-31.svg[{full-width}]

Taproot signaled in its first retarget period and locked in.
Activation followed at block height 709632 in November 2021.
