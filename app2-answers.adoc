...
in earlier output types are 71-73 bytes and variable-length, because
they are DER-encoded.

. The output key is the result of _tweaking_ the wallet’s internal
public key by a 32-byte value derived from the internal key alone
(if no script tree) or from the internal key combined with the
merkle root of the script tree (if there are scripts). Calling it
the “output key” rather than “the public key” makes the distinction
clear: the wallet still has its internal key P; the chain only sees
the tweaked output key Q.
...
.. Recomputes the output key by tweaking P with the hash of P
combined with the reconstructed merkle root.
.. Checks that the recomputed value equals the actual output key in
the spent output.
